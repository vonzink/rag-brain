 ...
 *
 * <p>Timeouts are configurable via {@code msfg.rag.ai.connect-timeout-ms} and
 * {@code msfg.rag.ai.read-timeout-ms}. The read timeout must comfortably exceed a
...
public class AiHttpClientFactory {
    ...
    public AiHttpClientFactory(
            @Value("${msfg.rag.ai.connect-timeout-ms:10000}") long connectTimeoutMs,
            @Value("${msfg.rag.ai.read-timeout-ms:60000}") long readTimeoutMs) {
